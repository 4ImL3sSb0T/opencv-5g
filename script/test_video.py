...
if __name__ == "__main__":
    ...
    # 标签字典
    dic_labels= {0:'A',1:'B',2:'left',3:'right'}
    ...
    while True:
        ...
        if success:
            ...
            if flag_det:
                t1 = time.time()
                det_boxes,scores,ids = infer_img(img0,net,model_h,model_w,nl,na,stride,anchor_grid,thred_nms=0.4,thred_cond=0.78)
                t2 = time.time()
                ...
                time_diff = t2 - t1 # 计算FPS
                if time_diff <= 0:# 防止除0错误
                    time_diff = 0.001    # 防止除0错误
                str_FPS = "FPS: %.2f"%(1./(time_diff))
