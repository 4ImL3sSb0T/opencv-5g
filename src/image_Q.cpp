                      ...
                      int16* Right_Line, int16* Left_Add_Line, int16* Right_Add_Line, int mods)
{
    ...
    {
        if (Road_Width_Real[k] > 50 && Road_Width_Real[k] < 300)
        {
            // 过滤异常值
...
cv::VideoWriter writer("output288.avi", cv::VideoWriter::fourcc('X', 'V', 'I', 'D'), 60, cv::Size(320, 96));
// 创建视频写入对象，指定输出文件、编码器、帧率和分辨�?
...
int count_valid_lines(int line_array[], int total_rows)
{
    ...
    for (int i = 0; i < total_rows; i++)
    {
        ...
        if (line_array[i] > 0 && line_array[i] < 320)
        {
            // 根据你的图像宽度调整（320是示例）
            valid_count++;
        }
...
int BanMa_Find111(cv::Mat BanMa_Find_data)
{
    ...
    pingJun = (Q_jdz(pingJun - Y_point[1]) + Q_jdz(pingJun - Y_point[2]) + Q_jdz(pingJun - Y_point[3]) + Q_jdz(
        pingJun - Y_point[4])) / 4;
