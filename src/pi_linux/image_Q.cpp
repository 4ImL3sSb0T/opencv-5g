...
//扫线
int bizhanground=0;
int bizhangenable=0;//默认0为不允许，1为允许
int yellowenable=0;
int BZget;
int stopbanma=0;
// error突变保护相关变量
...
//////////////////////////避障处理部分//////////////////////////////
if(bizhangenable==1 && bizhangclock == 1)
{           int BZ_GET = 0;
...
cv::Mat red_mask;
void BZ_LuoJISET(void)//避障逻辑//2025.11.21修改
{   

    static int zhangai_flag1 = 0;
    static int zhangai_flag22 = 1;
    static int zhangai_Left_or_Right = 1;
    static int nums = 0;
    std::cout << "find_XYdata[1] 的值为: " << find_XYdata[1] << std::endl;
    if (find_XYdata[1] > 520 && zhangai_flag1 == 0)//在图像下方出现
    {
       nums++;
       if(nums >= 1)//3帧 //应该是可变的
        {
    ...
    if (zhangai_flag1 == 1)
    {   //根据看到地面标志方向选择避障方向，如果没看到标志（leftrightflag == 3）默认右转
        //注意这里左右避障逻辑并没有写反，箭头指向左时会先向左换一次道再通过避障算法变道变回原赛道，右侧同理
...
//蓝色阈值
    ...
    //黄色图像
    cv::Mat yellow_mask;
    std::vector<std::vector<cv::Point>> yellow_contours; // 添加黄色轮廓
...
// S: 100-255 (过滤白色)
// V: 50-255 (过滤太暗)
    cv::Scalar blue_lower(100, 100, 90);   // 蓝色下限
    cv::Scalar blue_upper(130, 255, 255);  // 蓝色上限
...
///黄色范围
    ...
    // 修改前：这里设置成了红色色域 (H: 142-179)，导致误触发
    // cv::Scalar yellow_lower(142,45, 55);
    // cv::Scalar yellow_upper(179, 255, 255);//晚上

    // 修改后：改为黄色的色域 (建议使用之前注释掉的参数或标准黄色范围)
    cv::Scalar yellow_lower(15, 43, 46); 
